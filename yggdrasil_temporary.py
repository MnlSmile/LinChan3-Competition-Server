...
async def login(data:YggdrasilLoginRequestData) -> dict:
    resp = {
        "accessToken": (ak := str(_uuid.uuid4())),
        "clientToken": data.clientToken,
        "availableProfiles": [
            temp_profile := {
                "id": str(_uuid.uuid4()).replace('-', ''),
                "name": data.username,
                "properties": []
            }
        ],
        "selectedProfile": temp_profile,
        "user": {
            "id": str(_uuid.uuid4()).replace('-', ''),
            "properties": [
                {
    ...
    uuid_to_name[temp_profile['id']] = data.username
    name_to_ak[data.username] = temp_profile["id"]
    ak_to_user[ak] = temp_profile
    return resp
...
async def refresh(data:YggdrasilRefreshRequestData) -> dict:
    return {
        ...
        "clientToken":data.clientToken,
        "selectedProfile": ak_to_user[data.accessToken],
        "user": {
...
async def profile(uuid:str, unsigned:bool=False) -> dict:
    texture = {
        ...
        "profileId": uuid,
        "profileName": (name := uuid_to_name[uuid]),
        "textures": {
            "SKIN": {
                "url": "http://47.119.20.145:5799/LinChan3-Competition/api/v1/yggdrasil/e051c27e803ba15de78a1d1e83491411dffb6d7fd2886da0a6c34a2161f7ca99"
            },
            #"CAPE": {
